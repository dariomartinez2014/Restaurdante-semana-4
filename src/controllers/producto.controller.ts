...
export async function postProducto(req: Request, res: Response) {
  ...
  */
  try {
    ...
    const newProduct = await ProductoModel.create({ nombre, categoria, precio, disponibilidad });
...
export async function putProducto(req: Request, res: Response) {
  ...
  try {
    const id = Number(req.params.id);
    if (isNaN(id)) {
      res.status(400).json({ error: "El ID ingresado debe ser un valor numerico" });
    }
